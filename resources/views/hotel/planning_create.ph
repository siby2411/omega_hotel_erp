<div class="card bg-dark text-white p-4 shadow">
    ...
    <form action="?url=collab_save_planning" method="POST">
        <div class="mb-3">
            ...
            <select name="employe_code" class="form-select bg-secondary text-white" required>
                <?php foreach($personnel as $e): ?>
                    <option value="<?= $e['code_employe'] ?>"><?= $e['nom'] ?> <?= $e['prenom'] ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="type_rotation" class="form-control bg-secondary text-white" placeholder="ex: Service Caisse" required>
        </div>
        <button type="submit" class="btn btn-primary">Enregistrer au Planning</button>
    </form>
</div>
